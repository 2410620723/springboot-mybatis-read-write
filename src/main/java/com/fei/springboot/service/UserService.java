...
public class UserService {
	...
	@CachePut(value="user", key="#root.targetClass + #u.id",unless="#u eq null")
	public void insertUser(User u){
		this.userMapper.insert(u);
		System.out.println("若此句不输出，则表示缓存生效");
		//如果类上面没有@Transactional,方法上也没有，哪怕throw new RuntimeException,数据库也会成功插入数据
	//	throw new RuntimeException("测试插入事务");
	}
	@WriteDataSource
	@Transactional(propagation=Propagation.REQUIRED,isolation=Isolation.DEFAULT,readOnly=false)
	@CachePut(value="user", key="#root.targetClass + #user.id",unless="#user eq null")
